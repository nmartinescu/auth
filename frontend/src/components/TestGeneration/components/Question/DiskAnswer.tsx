...
import type { DiskTestSolution } from "../../types";

interface DiskAnswerProps {
    diskSequence: number[];
    maxDiskSize: number;
...
const DiskAnswer = ({
    ...
    const handleSequenceChange = (index: number, value: string) => {
        ...
        const newSequence = diskSequence.map((pos, idx) => 
            idx === index ? numValue : pos
        );
        onDiskSequenceChange(newSequence);
    };
                    ...
                    <VStack align="start" gap={3}>
                        <VStack align="start" gap={2}>
                            <Text fontSize="sm" color={textColor}>
                                Enter the order in which disk requests are serviced. The first position is the initial head position ({initialHeadPosition}). 
                                You need to enter the remaining {requests.length} positions for the requests: [{requests.join(', ')}].
                                Seek times will be calculated automatically as you enter the sequence.
                            </Text>
                            <Text 
                                fontSize="xs" 
                                color={isComplete ? "green.600" : "orange.600"}
                                fontWeight="medium"
                            >
