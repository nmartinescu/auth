...
import type { ProcessResult, TestSolution } from "../../types";

interface SchedulingAnswerProps {
...
const SchedulingAnswer = ({
    ...
    const handleProcessFieldChange = (pid: number, field: string, value: string) => {
        ...
        const updatedResults = processResults.map(p => 
            p.pid === pid ? { ...p, [field]: numValue } : p
        );
        onProcessResultsChange(updatedResults);
    };
                    ...
                    return (
                        ...
                        >
                            <GridItem p={3} borderRight="1px" borderColor={borderColor} display="flex" alignItems="center">
                                <Text color={primaryTextColor} fontWeight="medium">P{process.pid}</Text>
                            </GridItem>
                            <GridItem p={3} borderRight="1px" borderColor={borderColor} display="flex" alignItems="center">
                    ...
                    <Box>
                        ...
                        {reviewMode && correctSolution ? (
                            <VStack align="start" gap={1}>
                                <Text 
                                    fontSize="lg" 
                                    color={Math.abs(avgWaitingTime - correctSolution.avgWaitingTime) <= 0.1 ? "green.600" : "red.600"}
                                    fontWeight="semibold"
                                >
                                    Your: {avgWaitingTime}
                                </Text>
                    ...
                    <Box>
                        ...
                        {reviewMode && correctSolution ? (
                            <VStack align="start" gap={1}>
                                <Text 
                                    fontSize="lg" 
                                    color={Math.abs(avgTurnaroundTime - correctSolution.avgTurnaroundTime) <= 0.1 ? "green.600" : "red.600"}
                                    fontWeight="semibold"
                                >
                    ...
                    <Box>
                        ...
                        {reviewMode && correctSolution ? (
                            <VStack align="start" gap={1}>
                                <Text 
                                    fontSize="lg" 
                                    color={Math.abs(completionTime - correctSolution.completionTime) <= 0.1 ? "green.600" : "red.600"}
                                    fontWeight="semibold"
                                >
                                    Your: {completionTime}
                                </Text>
