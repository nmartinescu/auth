...
import type { MemoryAnswerProps } from "../../types";
...
const MemoryAnswer = ({
        ...
        onMemoryStepsChange(
            memorySteps.map((step, idx) =>
                idx === stepIndex
                    ? {
                          ...step,
                          frameState: step.frameState.map((f, fIdx) =>
                              fIdx === frameIndex ? numValue : f
                          ),
                      }
                    : step
            )
    ...
    const hitRate =
        memorySteps.length > 0
            ? (
                  ((memorySteps.length - totalPageFaults) /
                      memorySteps.length) *
                  100
              ).toFixed(1)
            : 0;
