...
class ApiService {
  ...
  async post(url: string, data: unknown, requiresAuth = false): Promise<Response> {
  ...
  async put(url: string, data: unknown, requiresAuth = false): Promise<Response> {
    return this.request(url, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
  ...
  async delete(url: string, data?: unknown, requiresAuth = false): Promise<Response> {
